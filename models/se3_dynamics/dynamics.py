...
class OurDynamics(torch.nn.Module):
    def __init__(self, nf=16, n_layers=3, act_fn=nn.ReLU(), model="se3_transformer", num_degrees=4, div=1):
        super().__init__()
    ...
    def f(self, xs, vs, charges, edge_index):
        ...
        device = xs.device
        self.graph = dgl.graph((edge_index[0], edge_index[1]), num_nodes=xs.size(0)).to(device)
