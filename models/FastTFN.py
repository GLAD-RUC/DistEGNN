...
class TFN_GCL_vel(nn.Module):
    ...
    def __init__(self, node_feat_nf, node_feat_out_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels, num_degrees=2, 
        ...
        # Single Layer TFN
        self.tfn_proj = nn.Sequential(
            nn.Linear(hidden_nf, hidden_nf),
            act_fn,
            nn.Linear(hidden_nf, 1)
        )
        # self.tfn_layer = TFN(num_layers=2, num_channels=hidden_nf // 2, edge_dim=0, div=1, act_fn=act_fn, num_degrees=2)
    ...
    def coord_model_vel(self, node_feat, coord, node_vel, edge_index, virtual_edge_feat, virtual_coord_diff, charges):
        def coord_model_by_tfn(xs, vs, charges, edge_index):
            device = xs.device
            graph = dgl.graph((edge_index[0], edge_index[1]), num_nodes=xs.size(0)).to(device)
            graph.ndata['x'] = xs
            graph.ndata['vel'] = vs.unsqueeze(1)
            graph.ndata['f'] = charges.unsqueeze(2)
            graph.ndata['f1'] = graph.ndata['vel']
            
            graph.edata['d'] = xs[edge_index[1]] - xs[edge_index[0]]
        ...
        trans_r = coord_model_by_tfn(coord, node_vel, charges, edge_index)
        coord = coord + trans_r
    ...
    def forward(self, node_feat, edge_index, coord, node_vel, virtual_coord, virtual_node_feat, data_batch, charges, edge_attr=None, node_attr=None):
        row, col = edge_index
        ...
        coord = self.coord_model_vel(node_feat, coord, node_vel, edge_index, virtual_edge_feat, virtual_coord_diff, charges)
...
class FastTFN(nn.Module):
    ...
    def forward(self, node_feat, node_loc, node_vel, loc_mean, edge_index, data_batch, charges, edge_attr=None, node_attr=None):
        ...
        for i in range(0, self.n_layers):
            node_feat, node_loc, virtual_node_feat, virtual_node_loc = \
                  self._modules["gcl_%d" % i](node_feat, edge_index, node_loc, node_vel, virtual_node_loc,
                   virtual_node_feat, data_batch, charges, edge_attr=edge_attr, node_attr=node_attr)
