...
class SchNet_GCL_vel(nn.Module):
    """
    E(n) Equivariant Convolutional Layer (With SchNet)
    """

    def __init__(self, node_feat_nf, node_feat_out_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels, cutoff, 
                 act_fn=nn.SiLU(), residual=True, attention=False, normalize=False, coords_agg='mean', tanh=False, gravity=None):
        super(SchNet_GCL_vel, self).__init__()
        ...
        # Single SchNet Layer
        self.schnet_layer = SchNet(hidden_channels=hidden_nf, max_num_neighbors=200000, cutoff=cutoff, num_interactions=1)
    ...
    def coord_model_vel(self, node_feat, coord, node_vel, edge_index, virtual_edge_feat, virtual_coord_diff, data_batch):
        def coord_model_by_schnet(node_feat, coord, data_batch, edge_index):
            return self.schnet_layer(z=node_feat, pos=coord, batch=data_batch, edge_index=edge_index, embedding=False)
        
        coord = coord_model_by_schnet(node_feat, coord, data_batch, edge_index) 
        # Message from virtual nodes
    ...
    def forward(self, node_feat, edge_index, coord, node_vel, virtual_coord, virtual_node_feat, data_batch, edge_attr=None, node_attr=None):
        ...
        # Coord Model (Real nodes update using tfn layer)
        coord = self.coord_model_vel(node_feat, coord, node_vel, edge_index, virtual_edge_feat, virtual_coord_diff, data_batch)
...
class FastSchNet(nn.Module):
    def __init__(self, node_feat_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels, cutoff, device='cpu', act_fn=nn.SiLU(), 
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, SchNet_GCL_vel(hidden_nf, hidden_nf, node_attr_nf, edge_attr_nf, hidden_nf, virtual_channels=virtual_channels, cutoff=cutoff,
    ...
    def forward(self, node_feat, node_loc, node_vel, loc_mean, edge_index, data_batch, edge_attr=None, node_attr=None):
        ...
        for i in range(0, self.n_layers):
            node_feat, node_loc, virtual_node_feat, virtual_node_loc = \
                  self._modules["gcl_%d" % i](node_feat, edge_index, node_loc, node_vel, virtual_node_loc,
