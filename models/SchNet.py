"""
    SchNet: A continuous-filter convolutional neural network for modeling quantum interactions
        From: PyG. 
        Add equivariant coordinate updation each layer.
"""
...
class SchNet(torch.nn.Module):
    ...
    def forward(self, z: Tensor, pos: Tensor, edge_index,
                batch: OptTensor = None, embedding: bool = True) -> Tensor:
        r"""
        Args:
        ...
        for interaction, coord_update in zip(self.interactions, self.coord_updates):
            ...
            out_aggr = torch.zeros_like(pos)
            scatter_mean(aggr, index=row.unsqueeze(-1).repeat(1, 3), out=out_aggr, dim=0)
