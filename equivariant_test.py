...
model = FastEGNN(node_feat_nf=1, node_attr_nf=0, edge_attr_nf=1, hidden_nf=64, virtual_channels=3, 
                 world_size=1, act_fn=nn.SiLU(), n_layers=4, residual=True, attention=False, 
...
loc_mean = torch.mean(coordinates, dim=0).unsqueeze(0)
...
loc_mean_r = torch.mean(coordinates_r, dim=0).unsqueeze(0)
