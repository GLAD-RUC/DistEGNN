...
MAX_FLUID_START_VELOCITY_XZ = 2.0 * 2
MAX_FLUID_START_VELOCITY_Y = 0.5 * 2
...
def find_valid_fluid_start_positions(box_rasterized, fluid_rasterized):
    ...
    for idx in tqdm(itertools.product(range(0, last_pos[0] + 1),
                                 range(0, last_pos[1] + 1),
                                 range(0, last_pos[2] + 1)), total=(last_pos[0] + 1) * (last_pos[1] + 1) * (last_pos[2] + 1),
    ...
    selected_pos = selected_pos.astype(np.float32) * box_rasterized[1]
...
def create_fluid_data(output_dir, seed, options):
    ...
    def create_fluid_object():
        ...
        fluid = obj_volume_to_particles(fluid_obj,
                                        scale=np.random.uniform(0.90, 1.00))[0]  # TODO: Change Scale
    ...
    print(f'Total number of fluid particles: {get_total_number_of_fluid_particles()}')
    assert get_total_number_of_fluid_particles() > 100000, get_total_number_of_fluid_particles()
