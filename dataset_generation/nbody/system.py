...
class System:
    def __init__(self, n_isolated, n_stick, n_hinge, delta_t=0.001,
        ...
        """
            2024.09.20 -- GPT: sample nodes from multiple clusters
        """
        if clusters == 1:
            cluster_centers = np.array([0.0, 0.0, 0.0])
        elif clusters == 3:
            cluster_centers = np.random.uniform(low=-10 * clusters, high=10 * clusters, size=(clusters, self.dim))
        elif clusters == 10:
            cluster_centers = np.random.uniform(low=-3 * clusters, high=3 * clusters, size=(clusters, self.dim))
        else:
            assert False
            cluster_centers = np.random.uniform(low=-10 * math.pow(clusters, 1 / 3), high=10 * math.pow(clusters, 1 / 3), size=(clusters, self.dim))
        # print(cluster_centers)
        X = np.zeros((n, self.dim))
        
        # each node belongs to a random cluster
        for i in range(n):
            cluster_idx = np.random.choice(clusters)
            X[i] = np.random.randn(self.dim) * self.loc_std + cluster_centers[cluster_idx]
